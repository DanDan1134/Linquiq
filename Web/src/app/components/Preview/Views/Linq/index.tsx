...
const LinqChildCard = ({ file }: { file: EntryRow }) => {
...
const LinqPreview = ({ linq_data }: { linq_data: FileUrlResult[] | undefined }) => {
    useFileStore((state)=>state.layoutState)

    return (
        <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "1rem" }}>
            {linq_data?.map((item, index) => (
                isEntryRow(item.data)
                    ? <LinqChildCard key={item.data.id ?? index} file={item.data} />
                    : null
            ))}
        </div>
    )
